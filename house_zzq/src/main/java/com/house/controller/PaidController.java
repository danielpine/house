...
public class PaidController {
	...
	@RequestMapping(value = "/getpaids", method = RequestMethod.GET)
	public Result getpaids(@RequestParam("orderid") String orderid) {
		return new Result(true, StatusCode.SUCCESS, "查找租金信息列表成功", paidService.listByOrderId(orderid));
	}

	@RequestMapping(value = "/getallpaidlist", method = RequestMethod.GET)
	public Result getAllPaidList() {
		return new Result(true, StatusCode.SUCCESS, "查找租金信息列表成功",
				paidService.findPaidListByCondition(null, null, null, null));
	}
	...
	public Result getPaidListByCondition(@RequestBody Paid paid) {
		return new Result(true, StatusCode.SUCCESS, "按条件查找租金信息列表成功", paidService
				.findPaidListByCondition(paid.getStatus(), paid.getName(), paid.getAddress(), paid.getUserlist_id()));
	}
