...
public class HouseController {
	...
	public Result search(@RequestBody HouseList houseList) {
		return new Result(true, StatusCode.SUCCESS, "查找房屋信息列表成功", houseRepository.search(houseList.getCity()));
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.GET)
	public Result detail(@PathVariable Integer id) {
		return new Result(true, StatusCode.SUCCESS, "查找房屋信息列表成功", houseListService.findHouseById(id));
	}
	...
	public Result getHouseListByCondition(@RequestBody HouseList houseList) {
		return new Result(true, StatusCode.SUCCESS, "按条件查找房屋信息列表成功", houseListService
				.findHouseListByCondition(houseList.getStatus(), houseList.getAddress(), houseList.getUserlist_Id()));
	}
